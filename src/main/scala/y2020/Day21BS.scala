...
class Day21BS extends AdventOfCode with RegexParsers {
  ...
  override def calculate(): String = {
    ...
    var allergenIngredients: List[(String, String)] = Nil
    ...
    while (oldPossible != newPossible) {
      ...
      newPossible.filter(_._2.size == 1)
        .foreach(a => {
          ...
          allergenIngredients = (a._1, chosen) :: allergenIngredients
          newPossible = newPossible.filterKeys(_ != a._1)
            .mapValues(_.filterNot(_ == chosen))
        })
    }

    allergenIngredients.sortBy(_._1).map(_._2).mkString(",")
  }
